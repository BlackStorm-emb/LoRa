...
static uint16_t window_x0, window_x1, window_y0, window_y1;
...
inline void ST7735_SetData(void) {
...
void ST7735_Start(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1) {
	...
	if (window_x0 - window_x1 + 1 > ST7735_WIDTH) window_x1 = ST7735_WIDTH - 1;
	if (window_y0 - window_y1 + 1 > ST7735_HEIGHT) window_y1 = ST7735_HEIGHT - 1;
	//Start DMA
	...
	ST7735_SetData();
    HAL_SPI_Transmit_DMA(&hspi1, (uint8_t *)framebuffer, sizeof(uint16_t)* (x1 - x0) * (y1 - y0));
}
...
void ST7735_FillScreen(uint16_t color) {
    for (size_t i = 0; i < ST7735_WIDTH * ST7735_HEIGHT)
		framebuffer[i] = color;
}

void ST7735_DrawImage(uint8_t x, uint8_t y, uint8_t h, uint8_t w, const uint16_t* data) {
	if (x + h > window_x1 - window_x0 + 1) return;
	if (y + w > window_y1 - window_y0 + 1) return;
