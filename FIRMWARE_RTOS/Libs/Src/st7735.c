...
void ST7735_Init() {
    ...
    HAL_GPIO_WritePin(ST7735_DC_GPIO_Port, ST7735_DC_Pin, GPIO_PIN_SET);
    HAL_SPI_Transmit_DMA(&hspi1, (uint8_t *)framebuffer, sizeof(uint16_t)*ST7735_WIDTH*ST7735_HEIGHT);
    /*
...
void ST7735_DrawLine(uint16_t x1, uint16_t y, uint16_t x2) {
	while(x1 <= x2) {
		ST7735_DrawPixel(x1, y, ST7735_BLACK);
		ST7735_DrawPixel(x1, y+1, ST7735_BLACK);
		ST7735_DrawPixel(x1, y+2, ST7735_BLACK);
		x1++;
	}
}
...
void ST7735_FillScreen(uint16_t color) {
	...
	for (size_t i = 0; i < ST7735_HEIGHT * ST7735_WIDTH; i++)
		framebuffer[i] = color;
...
void ST7735_DrawImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t* data) {
    ...
    ST7735_DC_GPIO_Port->BSRR = ST7735_DC_Pin;
    //HAL_SPI_Transmit_DMA(&hspi1, (uint8_t *)data, sizeof(uint16_t)*w*h);
    HAL_SPI_Transmit_DMA(&hspi1, (uint8_t *)data, sizeof(uint16_t)*w*h);
    ST7735_Unselect();
}
